...
import com.hhsfbla.library.model.BooksOut;
import com.hhsfbla.library.model.Member;
import com.hhsfbla.library.view.BooksReportController;
import com.hhsfbla.library.view.MemberEditDialogController;
import com.hhsfbla.library.view.MemberLimitsController;
import com.hhsfbla.library.view.MembersInformationController;
import com.hhsfbla.library.view.MenuBarController;
import com.hhsfbla.library.view.WeeklyBooksReportController;
...
public class Main extends Application {
	 ...
	 * Enters password for database, which can only be accessed with the correct password
	 * 
	 */
	...
	private void createSchema(Connection con) throws SQLException {
		...
		String table = "create table member(firstname varchar(64), lastname varchar(64), type varchar(64))";
		st.executeUpdate(table);
		
		st.executeUpdate("INSERT into member values ('Andrew', 'Johnson', 'Student')");
		st.executeUpdate("INSERT into member values ('John', 'Smith', 'Student')");
		st.executeUpdate("INSERT into member values ('Samantha', 'Worthington', 'Student')");
		st.executeUpdate("INSERT into member values ('Peter', 'Brown', 'Teacher')");
		st.executeUpdate("INSERT into member values ('Bob', 'Anderson', 'Student')");
		st.executeUpdate("INSERT into member values ('Matthew', 'Thompson', 'Teacher')");
		st.executeUpdate("INSERT into member values ('Billy', 'Allen', 'Student')");
		st.executeUpdate("INSERT into member values ('Angela', 'Wright', 'Student')");
		st.executeUpdate("INSERT into member values ('Lilly', 'White', 'Teacher')");
		st.executeUpdate("INSERT into member values ('Daniel', 'Brown', 'Student')");
	...
	private boolean schemaExists(Connection con) {
		...
		try {
			Statement st = con.createStatement();
//			st.executeQuery("select count(*) from member");
	...
	public void start(Stage primaryStage) {
		...
		//Sets stage name to Lex Library Database App
		this.primaryStage.setTitle("Lex Library Database App");
		//Sets stage image, top bar
		this.primaryStage.getIcons().add(new Image("file:resources/main-icon.png"));
		//Sets the window to non-resizable
	...
	public void showBooks() {
		try{
			...
			//Sets the stage
			Stage dialogStage = new Stage();
			dialogStage.setTitle("Master Books Report"); //Sets title
			dialogStage.initModality(Modality.WINDOW_MODAL);
			dialogStage.initOwner(primaryStage);
			Scene scene = new Scene(page);
			dialogStage.setScene(scene);
